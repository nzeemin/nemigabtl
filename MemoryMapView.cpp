...
void MemoryMapView_Create(int x, int y)
{
    ...
    g_hwndMemoryMap = CreateWindowEx(
            ...
            CLASSNAME_OVERLAPPEDWINDOW, _T("Nemiga Memory Map"),
...
void MemoryMapView_InitBitmap()
{
    ...
    m_bmpinfoMemoryMap.bmiHeader.biHeight = 512;
...
void MemoryMapView_OnDraw(HDC hdc)
{
    ...
    DrawDibDraw(m_hMemoryMapDrawDib, hdc,
            ...
            256 * m_nMemoryMap_scale, 512 * m_nMemoryMap_scale,
...
void MemoryMapView_PrepareBitmap()
{
    ...
    {
        DWORD* pBits = m_pMemoryMap_bits + (512 - 1 - y) * 256;
        for (int x = 0; x < 256; x += 2)
        {
            ...
            if (y < 256)
            {
                ...
                value = g_pBoard->GetWordView(address, FALSE, FALSE, &addrtype);
            }
            else
            {
                WORD address = (WORD)(x + (y - 256) * 256);
                value = g_pBoard->GetHIRAMWord(address);
